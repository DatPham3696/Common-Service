...
import com.example.security_demo.application.dto.request.identity.Credential;
...
public class UserKeycloakService {
  ...
  public ResponseEntity<?> createKeycloakUser(RegisterRequest registerDTO) {
    return identityClient.createUser("Bearer " + token().getAccessToken(),
  ...
  public ResponseEntity<?> enableUser(String authorizationHeader, String userId,
      EnableUserRequest request) {
    return identityClient.enableUser(authorizationHeader, userId, request);
  }

  public ResponseEntity<?> resetPassword(String authorizationHeader, String userId,
