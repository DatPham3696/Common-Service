...
public class ExportDataController {
  ...
  @GetMapping("/export")
  public ResponseEntity<InputStreamResource> exportUserData(
      @ModelAttribute UserProfileSearchRequest request) throws IOException {
  ...
  public ResponseEntity<?> exportUserDataToStorage(
      @ModelAttribute UserProfileSearchRequest request) {
    try {
      Object result = exportDataService.exportDataToStorage(request);
      return ResponseEntity.ok(result);
    } catch (IOException e) {
      return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
          .body("Error exporting and uploading file: " + e.getMessage());
    }
  }
